...
const postData = async (priceUSD, pricePHP) => {
  try {
...
const convertCurrency = async (priceUSD) => {
  try {
    ...
    const pricePHP = priceUSD * data.conversion_rates.PHP;
    return pricePHP;
  } catch (error) {
...
cron.schedule("0 0 0 * * *", async () => {
  ...
  const pricePHP = await convertCurrency(priceUSD);
  postData(priceUSD, pricePHP);
});
