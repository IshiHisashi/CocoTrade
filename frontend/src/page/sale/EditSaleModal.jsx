    ...
    if (sale) {
      setFormData({
        ...sale,
            ...
            amount_of_copra_sold: parseFloat(sale.amount_of_copra_sold?.$numberDecimal ?? sale.amount_of_copra_sold),
            sales_unit_price: parseFloat(sale.sales_unit_price?.$numberDecimal ?? sale.sales_unit_price),
            copra_ship_date: sale.copra_ship_date ? new Date(sale.copra_ship_date).toISOString().split('T')[0] : '',
            cheque_receive_date: sale.cheque_receive_date ? new Date(sale.cheque_receive_date).toISOString().split('T')[0] : '',
            total_sales_price: parseFloat(sale.total_sales_price?.$numberDecimal ?? sale.total_sales_price),
        });
    }
